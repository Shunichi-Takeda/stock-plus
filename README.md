...
- メッセージへの **送信操作を自動検知** し、宛先（トーク相手・グループ名）を記録します。
  検知対象は「送信」ボタンのクリックと、入力欄での Cmd+Enter / Ctrl+Enter です。
- メッセージ一覧（チャットグループモーダル）の宛先名の横に **「✓ 返答済み」バッジ** を表示します。
- 画面左下のフローティングボタン（✓）から、**自身が返答したメッセージの宛先一覧**を
  時系列（新しい順）で確認できます。行クリックで該当スレッドを開きます
  （StockのメッセージはURLを持たないモーダルUIのため、宛先名の一致でスレッドを選択します）。
